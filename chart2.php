<?php
	...
	try {
		$sql = "select type, hour(dateOnline) as hourTime, sum(online) as online from user_online_history where date(dateOnline) >= '".$fromDate."' and date(dateOnline) <= '".$toDate."' and type like 's2%' group by type,hour(dateOnline) order by hour(dateOnline)";
		$users = array();
		foreach ($db->query($sql) as $row) {
			$type = $row['type'];
			if (startsWith($type, "s2")) {
				$type = substr($type,3);
			}
