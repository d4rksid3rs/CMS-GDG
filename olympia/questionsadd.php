...
// Create page object
$questions_add = new cquestions_add();
$Page =& $questions_add;

// Page init
$questions_add->Page_Init();

// Page main
$questions_add->Page_Main();
?>
...
// Create page object
var questions_add = new ew_Page("questions_add");

// page properties
questions_add.PageID = "add"; // page ID
questions_add.FormID = "fquestionsadd"; // form ID
var EW_PAGE_ID = questions_add.PageID; // for backward compatibility

// extend page with ValidateForm function
questions_add.ValidateForm = function(fobj) {
...
questions_add.Form_CustomValidate =  
...
questions_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
questions_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $questions->TableCaption() ?><br><br>
...
?>
<form name="fquestionsadd" id="fquestionsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return questions_add.ValidateForm(this);">
<p>
...
class cquestions_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $questions->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$questions->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $questions->CurrentAction = "C"; // Copy record
		  } else {
		    $questions->CurrentAction = "I"; // Display blank record
		...
		switch ($questions->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$questions->SendEmail = TRUE; // Send email on add success
		...
		$questions->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $questions;
		$questions->idLevel->CurrentValue = 0;
		$questions->Money->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		} elseif ($questions->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $questions->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($questions->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$questions->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $questions->id->DbValue;

			// Call Row Inserted event
			$questions->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
