...
// Create page object
$system_message_add = new csystem_message_add();
$Page =& $system_message_add;

// Page init
$system_message_add->Page_Init();

// Page main
$system_message_add->Page_Main();
?>
...
// Create page object
var system_message_add = new ew_Page("system_message_add");

// page properties
system_message_add.PageID = "add"; // page ID
system_message_add.FormID = "fsystem_messageadd"; // form ID
var EW_PAGE_ID = system_message_add.PageID; // for backward compatibility

// extend page with ValidateForm function
system_message_add.ValidateForm = function(fobj) {
...
system_message_add.Form_CustomValidate =  
...
system_message_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
system_message_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $system_message->TableCaption() ?><br><br>
<a href="<?php echo $system_message->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$system_message_add->ShowMessage();
?>
<form name="fsystem_messageadd" id="fsystem_messageadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return system_message_add.ValidateForm(this);">
<p>
...
<?php
$system_message_add->Page_Terminate();
?>
<?php
...
class csystem_message_add {
	...
	// Page object name
	var $PageObjName = 'system_message_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $system_message->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$system_message->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $system_message->CurrentAction = "C"; // Copy record
		  } else {
		    $system_message->CurrentAction = "I"; // Display blank record
		...
		switch ($system_message->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$system_message->SendEmail = TRUE; // Send email on add success
		...
		$system_message->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $system_message;
		$system_message->status->CurrentValue = 0;
		$system_message->message_type->CurrentValue = 0;
		$system_message->bonus->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		} elseif ($system_message->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// status
		$system_message->status->SetDbValueDef($rsnew, $system_message->status->CurrentValue, 0, TRUE);

		// message_type
		$system_message->message_type->SetDbValueDef($rsnew, $system_message->message_type->CurrentValue, 0, TRUE);

		// bonus
		$system_message->bonus->SetDbValueDef($rsnew, $system_message->bonus->CurrentValue, 0, TRUE);
		...
		$bInsertRow = $system_message->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($system_message->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$system_message->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $system_message->id->DbValue;

			// Call Row Inserted event
			$system_message->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
