...
$number = isset($_REQUEST['number']) ? $_REQUEST['number'] : die();
$fromDate = date('Y-m-d', strtotime($date."-${number} day"));
$toDate = date('Y-m-d', strtotime($date."-${number} day"));
...
<?php
	...
	foreach ($revenue as $row) {
		try {
			$sql = "insert revenue(date_created, type, partner,k2,mv,total) values('{$fromDate}','{$row['type']}','{$row['key']}','{$row['k2']}','{$row['mv']}','{$row['tong']}')";
			$db->exec($sql);
		} catch (Exception $e) {
			$failQuery .= $sql."<br>";
		} 
	...
	$t = "Doanh thu BEME ".$fromDate;
	$b = "<font size='16'><b>Doanh thu:<b></font> <font size='16' color='red'>".number_format($totalMoney)."</font><br><br><a href='http://bem.phang.mobi/bi1/stat_money.php?fromDate=${fromDate}&toDate=${toDate}'>http://bem.phang.mobi/bi1/stat_money.php</a>";
	$b .= "<br>".$failQuery;
	domail($t, $b);

	function domail($title, $body){
		...
		$mail->SetFrom("lamnhanvodanh@gmail.com", "Revenue Notify");

		$mail->AddReplyTo("lamnhanvodanh@gmail.com", "Revenue Notify");
		...
		$mAddress = explode(",", "ha@grepgame.com,quan@grepgame.com,bao@grepgame.com,kien@grepgame.com"); 
		$mName = explode(",", "Ha,Quan,Bao,Kien"); 
