<?php
	...
	if (isset($cp)) {
		$cp = strtolower($cp);
		$sql = "select * from config c where c.key='".$cp."'";
		try {
			foreach ($db->query($sql) as $row) {
				$subcp = json_decode($row['value'])->{'cp'};
				$sms = json_decode($row['value'])->{'sms'};
				$version = json_decode($row['value'])->{'version'}->{'j2me'};
				$version_android = json_decode($row['value'])->{'version'}->{'android'};
				$version_iphone = json_decode($row['value'])->{'version'}->{'iphone'};
			}
		} catch (Exception $e) {
			echo $e->getMessage();
		}
...
<html>
	...
	<body>
		...
		<div style="margin-top:10px;">
			...
			?>
			<form action="editParentCP.php?id=<?php echo $cp;?>" method="POST">
				<div style="margin:3px; padding:0.4em;">
					...
					<div style="margin-top:5px;">
						<textarea id="subcp" style="width:900px; height:100px;" name="SUBCP" readonly><?php $output = "";
											foreach ($subcp as $cp) {
												$output = $output.", ".trim($cp);
											}								
											echo trim(substr($output, 1));?></textarea>
					</div>
					<div>
						<b>SMS</b>
					</div>
					<div style="margin-top:5px;">
						<table width="100%" cellspacing=0 cellpadding=0>
							<tr>
								...
								<td>
									ServiceCode <input type="text" id="version" style="width:200px;" value="<?php echo $sms->{"BEM_DK"}->{"ServiceCode"};?>" name="SMS_DK_SC"/>
							...
							<tr>
								<td>BEM_PWD</td>
								<td>
									ServiceCode <input type="text" id="version" style="width:200px;" value="<?php echo $sms->{"BEM_PWD"}->{"ServiceCode"};?>" name="SMS_PWD_SC"/>
							...
							<tr>
								<td>BEM_TANG</td>
								<td>
									ServiceCode <input type="text" id="version" style="width:200px;" value="<?php echo $sms->{"BEM_TANG"}->{"ServiceCode"};?>" name="SMS_TANG_SC"/>
							...
							<tr>
								<td>BEM_KOIN</td>
								<td>
									ServiceCode <input type="text" id="version" style="width:200px;" value="<?php echo $sms->{"BEM_KOIN"}->{"ServiceCode"};?>" name="SMS_KOIN_SC"/>
							...
							<tr>
								...
								<td>
									ServiceCode <input type="text" id="version" style="width:200px;" value="<?php echo $sms->{"BEM_NAP"}->{"ServiceCode"};?>" name="SMS_NAP_SC"/>
									<?php 
										$count = 1;
							...
							<tr>
								<td>BEM_ACTIVE</td>
								<td>
									ServiceCode <input type="text" id="active" style="width:200px;" value="<?php echo $sms->{"BEM_ACTIVE"}->{"ServiceCode"};?>" name="SMS_ACTIVE_SC"/>
