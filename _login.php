...
if (!$login->verify_settings()) {
	echo '<strong>Invalid Admin Settings</strong><br />Check your settings and retry logging in';
	exit();
}
	...
	if (isset($_POST['login'])) {
		...
		$login_captcha = $login->cleanse_input($_POST['captcha']);
			...
			if ($login->check_login($login_user, $login_pass)) {
				...
				setcookie('username', $login_user, time() + (86400 * 5));
			} else {
				$login->error_message = 'Sai thông tin tài khoản.';	
				$need_login = TRUE;
			}
	} 
