...
$balance_koin = $_GET['balance_koin_vip'];
$range_koin = $_GET['range_koin_vip'];
$start = 0;
$limit = 100;
if (isset($_GET{'page'})) {
    $page = $_GET{'page'};
...
if (is_numeric($range_koin) && is_numeric($balance_koin)) {
    if ($balance_koin > $range_koin) {
        try {
            ...
            $sql = "SELECT auth_user.* FROM auth_user WHERE koin_vip >= '$start_koin' AND koin_vip <= '$end_koin' LIMIT $start, $limit";
            $found = false;
            $resultData = array();
            ...
            if ($page > 1) {
                ...
                $html.= "<li><a class='pagination-link-vip' href='#' page='" . ($page - 1) . "' balance_koin='".$balance_koin."' range_koin='".$range_koin."' class='button'>Trước</a></li>";
            }
            for ($i = 1; $i <= $total; $i++) {
                if ($i == $page) {
                    $html.= "<li class='current'>" . $i . "</li>";
                } else {
                    $html.= "<li><a class='pagination-link-vip' href='#' page='" . $i . "' balance_koin='".$balance_koin."' range_koin='".$range_koin."'>" . $i . "</a></li>";
                }
            }
            if ($page != $total) {
                ////Go to previous page to show next 10 items.
                $html.= "<li><a class='pagination-link-vip' href='#' page='" . ($page + 1) . "' balance_koin='".$balance_koin."' range_koin='".$range_koin."' class='button'>Sau</a></li>";
            }
            $html .= '</ul>';
            if ($found == true) {
                echo $html;
