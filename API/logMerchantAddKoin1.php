...
if ($_GET['user']) {
    ...
    echo "<b>Phải chọn 1 Đại lý</b>";
    exit;
}
$fromDate = $_GET['from'];
$toDate = $_GET['to'];
...
try {
    ...
    $sql = "SELECT ma.*, m.screen_name, m.merchant_name FROM `merchant_add_koin` AS ma JOIN merchants AS m ON ma.username = m.username "
            . "WHERE date(date_created) >= '{$fromDate}' AND date(date_created) <= '{$toDate}' AND ma.username LIKE '{$username}' LIMIT $start, $limit ";
    $html = "<table width='100%'><tr style='background-color: rgb(255, 255, 255);text-align:center;'>";
    $html .= "<td>Tên Đại lý</td><td>Tên Hiển thị</td><td>VNĐ</td><td>Xu</td><td>Thời điểm</td><td>Người thêm</td>";
    $i = 0;
//    echo $sql;die;
    ...
    $rows = $db->query("SELECT ma.*, m.screen_name, m.merchant_name FROM `merchant_add_koin` AS ma JOIN merchants AS m ON ma.username = m.username "
            . "WHERE date(date_created) >= '{$fromDate}' AND date(date_created) <= '{$toDate}' AND ma.username LIKE '{$username}'");
    $count = $rows->rowCount();
    ...
    if ($page > 1) {
        ...
        $html.= "<li><a class='pagination-link-1' href='#' page='" . ($page - 1) . "' class='button'>Trước</a></li>";
    }
    for ($i = 1; $i <= $total; $i++) {
        if ($i == $page) {
            $html.= "<li class='current'>" . $i . "</li>";
        } else {
            $html.= "<li><a class='pagination-link-1' href='#' page='" . $i . "'>" . $i . "</a></li>";
        }
    }
    if ($page != $total) {
        ////Go to previous page to show next 10 items.
        $html.= "<li><a class='pagination-link-1' href='#' page='" . ($page + 1) . " class='button'>Sau</a></li>";
    }
    $html .= '</ul>';
    if ($found == true) {
        echo $html;
