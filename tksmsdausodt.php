...
<?php require_once('topMenu.sub2.php'); ?>
                <div class="box_header"><a href="javascript:void(0);">Thống kê tin nhắn từ đầu số đối tác</a></div>
                <div class="box_body">
                    <table width="100%">
...
/*
  $host="127.0.0.1:3307";
  $user="k2tek";
  $password="k2tek123456";
  mysql_connect($host,$user,$password);

  mysql_select_db("beme_cp") or die("Ko ket noi duoc toi CSDL 1");
 */
...
?>
                            ...
                            if ($cmd != NULL && $cmd == "Submit") {
                                ...
                                $sql5 = "SELECT count(*) AS count FROM auth_user_partner WHERE partner = '{$code}' and service in ('DK','NAP') AND telco in (1,2,3,-1) AND created_on BETWEEN '{$fDate}' AND '{$tDate}'";
    ...
    $sql = "SELECT count(*) AS count,telco,recipient FROM auth_user_partner WHERE partner = '{$code}' and service in ('DK','NAP') AND telco in (1,2,3,-1) AND created_on BETWEEN '{$fDate}' AND '{$tDate}' GROUP BY telco,recipient";
    //echo $sql;
